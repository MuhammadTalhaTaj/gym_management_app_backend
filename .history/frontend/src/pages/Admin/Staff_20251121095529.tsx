...
const StatCard = ({ stat }) => {
  ...
  return (
    <div className={`bg-white rounded-xl p-6 border-l-4 ${stat.borderColor} shadow-sm hover:shadow-md transition-shadow`}>
      <div className="flex items-start justify-between">
        <div>
          <p className="text-[#8C9BB0] text-sm font-medium mb-2">{stat.label}</p>
          <h3 className="text-[#1E293B] text-4xl font-bold">{stat.value}</h3>
        </div>
...
const SearchBar = ({ value, onChange }) => (
  <div className="relative flex-1 max-w-md">
    <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-[#8C9BB0]" />
    <input
      type="text"
      value={value}
      onChange={(e) => onChange(e.target.value)}
      placeholder="Search staff by name, email, or role..."
      className="w-full pl-12 pr-4 py-3 rounded-lg border border-[#8C9BB0]/30 focus:outline-none focus:ring-2 focus:ring-[#3D8BF2] focus:border-transparent text-[#1E293B] placeholder:text-[#8C9BB0]"
    />
  </div>
);
    ...
    <table className="w-full">
      <thead>
        <tr className="border-b border-[#8C9BB0]/20">
          <th className="text-left py-4 px-4 text-xs font-semibold text-[#8C9BB0] uppercase tracking-wider">Staff Member</th>
          <th className="text-left py-4 px-4 text-xs font-semibold text-[#8C9BB0] uppercase tracking-wider">Contact</th>
          <th className="text-left py-4 px-4 text-xs font-semibold text-[#8C9BB0] uppercase tracking-wider">Role</th>
          <th className="text-left py-4 px-4 text-xs font-semibold text-[#8C9BB0] uppercase tracking-wider">Permissions</th>
          <th className="text-left py-4 px-4 text-xs font-semibold text-[#8C9BB0] uppercase tracking-wider">Created</th>
          <th className="text-left py-4 px-4 text-xs font-semibold text-[#8C9BB0] uppercase tracking-wider">Status</th>
          <th className="text-left py-4 px-4 text-xs font-semibold text-[#8C9BB0] uppercase tracking-wider">Actions</th>
        </tr>
      </thead>
      <tbody>
        {data.map((staff) => (
...
const Staff = () => {
  ...
  return (
    <div className="min-h-screen bg-[var(--primary-100)]">
      {/* Header */}
      <header className="bg-[#364659] shadow-md">
        <div className="max-w-[1600px] mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="bg-[#EC9A0E] p-3 rounded-xl">
      ...
      <main className="max-w-[1600px] mx-auto px-6 py-8">
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <div className="p-6 border-b border-[#8C9BB0]/20">
            <h2 className="text-xl font-bold text-[#1E293B]">Staff Members</h2>
            <p className="text-[#8C9BB0] text-sm mt-1">Manage your team members and their permissions</p>
          </div>
